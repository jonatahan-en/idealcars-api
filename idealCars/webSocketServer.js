...
// import { createTransport} from "./lib/emailManager.js";
// export let io;
    ...
    io.on('connection', async (socket) => {
        ...
        socket.on('chat-message', async ({ text, adId }) => {
            try {
                ...
                // Enviar notificación por correo si el dueño no está en la sala
                if (!ownerInRoom && product.owner.toString() !== userId) {
                    const adOwner = await User.findById(product.owner);
                    if (adOwner?.email) {
                        const transporter = await createTransport();
                        await transporter.sendMail({
                            from: process.env.EMAIL_SERVICE_FROM,
                            to: adOwner.email,
                            subject: 'Nuevo mensaje en tu anuncio de IdealCars',
                            text: `Hola ${adOwner.name},\n\nTienes un nuevo mensaje en tu anuncio: "${product.name}".\nIngresa a IdealCars para responder.`,
                        });
                        console.log(`Correo de notificación enviado a ${adOwner.email}`);
                    }
                }
            } catch (error) {
                console.error('Error al procesar el mensaje:', error);
